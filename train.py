...
if __name__ == '__main__':
    ...
    model = YOLO('/root/projects/WormReIDTracker-all/ultralytics/cfg/models/yolov8-WormReIDTracker.yaml')
    ##Loading pre-trained models, and need to configure the path correctly.
    #model.load('CSB-1.pt')

model.train(
            data='coco8-developmental-worm.yaml',
            epochs=60,  # (int) Number of training epochs
            patience=2000,  # (int) Number of epochs to wait for early stopping without improvement
            batch=4,  # (int) Number of images per batch (-1 for auto batching)
            imgsz=1640, # (int) Size of input images, integer or w,h
            ...
            pretrained=True,  # (bool | str) Use pretrained model (bool), or load weights from model (str)
