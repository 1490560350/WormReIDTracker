...
from ultralytics.nn.backbone.EfficientNetV2 import *
from ultralytics.nn.backbone.RevColV1 import *
from ultralytics.nn.backbone.UNetV2 import *
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            if not args:
                raise ValueError(f"Layer {f} ({m}) has no arguments in the configuration.")
            c1, c2 = ch[f], args[0]
            if c2 != nc:  # if c2 not equal to number of classes (i.e. for Classify() output)
                c2 = make_divisible(min(c2, max_channels) * width, 8)
            if m is C2fAttn:
                if len(args) < 3:
                    raise ValueError(f"Layer {f} ({m}) expects at least 3 arguments.")
            ...
            if m in {BottleneckCSP, C1, C2, C2f, C2fAttn, C3, C3TR, C3Ghost, C3x, RepC3, C2fCIB}:
                ...
                n = 1
                   ...
                   starnet_s050, starnet_s100, starnet_s150, starnet_s1, starnet_s2, starnet_s3, starnet_s4,efficientnet_v2,revcol_small,
                   revcol_tiny,revcol_base, revcol_xlarge, revcol_large,
          ...
          #  if m is RevCol:
            ...
            if hasattr(m, 'channel'):
                c2 = m.channel
            elif hasattr(m, 'width_list'):
                c2 = m.width_list
        elif m is FusionShare:
            c1 = [ch[x] for x in f]
            c2 = int(c1[1] * 0.5 * 3)
            args = [c1, *args]
        elif m in {CSPSWC}:
        ...
        save.extend(
            x % (i + 4 if is_backbone else i) for x in ([f] if isinstance(f, int) else f) if x != -1)  # append to savelist
        layers.append(m_)
        if i == 0:
            ch = []
        if isinstance(c2, list):
